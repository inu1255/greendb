...
class Field {
    ...
    toString() {
        ...
        if (this._comment)
            s += ` COMMENT '${this._comment.replace(/'/g, "\\'")}'`;
        return s;
    }
    /**
    ...
    _param(primary_keys) {
        ...
        if (this._comment) {
            ...
            if (lbl.length > 8)
                ...
                v.lbl = lbl;
                if (ss.length > 1)
                    v.rem = this._comment.slice(lbl.length + 1);
            }
        }
        ...
        if (primary_keys && primary_keys.length) {
            if (!this._null && primary_keys.indexOf(this._name) < 0)
                v.need = primary_keys.length > 1 ? primary_keys : primary_keys[0];
        }
        return v;
    }
    /**
    ...
    toMock() {
        ...
        let m = /char\((\d+)\)/.exec(this._type);
        if (m)
            return utils.randomString(Math.floor(Math.random() * +m[1]));
...
class Constraint {
    ...
    toString() {
        ...
        if (this._type === "PRIMARY")
            return `PRIMARY KEY ${field}`;
        let name = use(this.name()._name);
        if (this._type === "UNIQUE")
            return `UNIQUE KEY ${name} ${field}`;
        if (this._type === "FOREIGN")
            return `CONSTRAINT ${name} FOREIGN KEY ${field} REFERENCES ${use(this._ref_table)} (${use(this._ref_field)})`;
    ...
    parse(str) {
        ...
        if (m) {
            ...
            if (m[4])
                this._name = m[4].replace(/`/g, "").trim();
            if (m[2])
                this._name = m[2].replace(/`/g, "").trim();
            if (m[7])
                this._ref_table = m[7].replace(/`/g, "").trim();
            if (m[8])
                this._ref_field = m[8].replace(/`/g, "").trim();
        }
        return this;
    }
    eq(b) {
        if (this._type != b._type)
            return false;
        if (this._type == "FOREIGN")
            return compare(this._field, b._field) && compare(this._ref_field, b._ref_field) && compare(this._ref_table, b._ref_table);
...
class Table {
    ...
    toString() {
        let tail = [")"];
        if (this._engine)
            tail.push("ENGINE=" + this._engine);
        if (this._inc)
            tail.push("AUTO_INCREMENT=" + this._inc);
        if (this._charset)
            tail.push("DEFAULT CHARACTER SET " + this._charset);
        if (this._comment)
            tail.push(`COMMENT='${this._comment.replace(/'/g, "''")}'`);
    ...
    toQueryParams(filter) {
        ...
        for (let item of this._fields) {
            if (item instanceof Field && (!filter || filter(item)))
                Object.assign(params, item.toQueryParam());
        }
        return params;
    }
    toMock() {
    ...
    toDart() {
        ...
        for (let item of this._fields) {
            if (item instanceof Field)
                ss.push(`    this.${utils.camelCase(item._name)},`);
        }
        ss.push(`  });`);
     ...
     */
    merge(db, run, drop) {
        return db.execSQL(`show create table ${use(this._name)}`, [], { ignore: true }).then(rows => {
            ...
            for (let f1 of this._fields) {
                ...
                if (f0) {
                    ...
                    if (f1 instanceof Field) {
                        ...
                        if (f1._update)
                            sqls.push(`update ${use(this._name)} set ${use(f1._name)}=${f1._update};`);
                    }
                    else
            ...
            for (let f of table._fields) {
                if (f instanceof Field) {
                    ...
                    drops.push(`alter table ${use(this._name)} drop PRIMARY KEY`);
                else if (f._type == "FOREIGN")
                    drops.push(`alter table ${use(this._name)} drop FOREIGN KEY ${use(f._name)}`);
                else {
                    let ok = true;
                    for (let f1 of this._fields) {
                    ...
                    if (ok)
                        drops.push(`alter table ${use(this._name)} drop INDEX ${use(f._name)}`);
                }
            }
            ...
            if (err.errno == 1146) {
                // table 不存在
                if (run)
                    return db.execSQL(this.toString());
...
class TableBuilder {
    ...
    merge(db, run) {
        ...
        for (let k in this.tables) {
            let v = this.tables[k];
            tables.push(v.merge(db, false, true).then(sqls => ({ name: k, sqls })));
        }
